            ...
            <div class="container">
                ...
                <div class="twitch-container">
                    <div class="twitch-player" style="width: ${streamWidth};">
                        <iframe
                            src="https://player.twitch.tv/?channel=${streaming.twitch.username}&parent=${window.location.hostname || 'localhost'}&autoplay=false&muted=false"
                            height="400"
                            width="100%"
                    ...
                    <div class="twitch-chat" style="width: 30%;">
                        <iframe
                            src="https://www.twitch.tv/embed/${streaming.twitch.username}/chat?parent=${window.location.hostname || 'localhost'}"
                            height="400"
                            width="100%"
                            frameborder="0"
                            scrolling="no">
...
class YouTubeAPI {
  ...
  async fetchLiveVideos() {
    ...
    try {
      ...
      const maxResults = Math.min(CONFIG.youtube.maxLiveVideos, 50);
      let allLiveVideos = [];
      let nextPageToken = '';
      let remainingVideos = CONFIG.youtube.maxLiveVideos;
      
      while (remainingVideos > 0 && (nextPageToken !== undefined || allLiveVideos.length === 0)) {
        const pageSize = Math.min(remainingVideos, 50);
        const pageUrl = \`\${this.baseUrl}/search?key=\${this.apiKey}&channelId=\${this.channelId}&part=snippet,id&order=date&maxResults=\${pageSize}&type=video&eventType=live\${nextPageToken ? \`&pageToken=\${nextPageToken}\` : ''}\`;
        ...
        const data = await response.json();
        allLiveVideos = [...allLiveVideos, ...data.items];
        nextPageToken = data.nextPageToken;
        remainingVideos -= data.items.length;
