...
class YouTubeAPI {
  ...
  try {
    // Fetch only past live streams using the search endpoint
    const url = \`\${this.baseUrl}/search?key=\${this.apiKey}&channelId=\${this.channelId}&part=snippet&eventType=completed&type=video&maxResults=\${CONFIG.youtube.maxLiveVideos}\`;
    const response = await fetch(url);
