...
class BasicInfoStep extends BaseSetupStep {
  ...
  async runStep() {
    const creatorName = await this.question(colors.cyan + 'Enter your creator name/brand: ' + colors.reset);
    this.setConfig('creatorName', creatorName);

    const websiteTitle = await this.question(colors.cyan + 'Enter your website title: ' + colors.reset);
    this.setConfig('websiteTitle', websiteTitle);

    const description = await this.question(colors.cyan + 'Enter a brief description of your content: ' + colors.reset);
