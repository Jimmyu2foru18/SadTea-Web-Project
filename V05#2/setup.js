...
async function setup() {
  ...
  const formOptions = [
    ...
    'Mailto (Simple email link - Backup option)',
    'No contact form (Skip this section)'
  ];
  
  displayOptions(formOptions, 'Choose your contact form service');
  
  const formChoice = await selectFromOptions(formOptions, 'Select option (1-4): ');
  ...
  if (formService !== 'none') {
    ...
    if (formService === 'formspree') {
      ...
      displayInfo('Mailto will create a simple email link that opens the user\'s default email client.');
      displaySuccess('Mailto contact form configured!');
    }
  } else {
  ...
  const themePresets = [
    { name: 'minimalist', primary: '#2c3e50', secondary: '#3498db', background: '#ffffff', text: '#2c3e50', accent: '#e67e22' },
    { name: 'vibrant', primary: '#e74c3c', secondary: '#27ae60', background: '#ffffff', text: '#2c3e50', accent: '#f39c12' },
    { name: 'dark', primary: '#3498db', secondary: '#2ecc71', background: '#2c3e50', text: '#ecf0f1', accent: '#f39c12' },
    { name: 'gradient', primary: '#8e44ad', secondary: '#3498db', background: '#ffffff', text: '#2c3e50', accent: '#e74c3c' },
    { name: 'gaming', primary: '#00ff88', secondary: '#00ccff', background: '#0a0a0a', text: '#ffffff', accent: '#ff0077' },
    { name: 'professional', primary: '#34495e', secondary: '#3498db', background: '#ffffff', text: '#2c3e50', accent: '#e67e22' }
  ];
            ...
            <div class="container">
                ...
                <div class="twitch-container">
                    <div class="twitch-player" style="width: ${streamWidth};">
                        <iframe
                            src="https://player.twitch.tv/?channel=${streaming.twitch.username}&parent=localhost&parent=127.0.0.1&parent=github.io&parent=pages.dev&parent=netlify.app&parent=vercel.app&autoplay=false&muted=false"
                            height="400"
                            width="100%"
                    ...
                    <div class="twitch-chat" style="width: 30%;">
                        <iframe
                            src="https://www.twitch.tv/embed/${streaming.twitch.username}/chat?parent=localhost&parent=127.0.0.1&parent=github.io&parent=pages.dev&parent=netlify.app&parent=vercel.app"
                            height="400"
                            width="100%"
                            frameborder="0"
                            scrolling="no">
...
function generateContactForm(contactForm) {
  if (contactForm.service === 'formspree') {
    ...
    return `
                    <div class="contact-form mailto-contact">
                        <div class="mailto-info">
                            <h3>Contact Me Directly</h3>
                            <p>Click the button below to send me an email using your default email client.</p>
                            <a href="mailto:${contactForm.email}?subject=Contact from Website" class="btn btn-primary mailto-btn">
                                <i class="fas fa-envelope"></i>
                                Send Email
                            </a>
                            <p class="mailto-email">Or email me directly at: <strong>${contactForm.email}</strong></p>
                        </div>
                    </div>`;
  }
  return '';
...
/* Header */
.header {
  background-color: ${config.theme.style === 'dark' || config.theme.style === 'gaming' ? 'rgba(44, 62, 80, 0.95)' : 'rgba(255, 255, 255, 0.95)'};
...
.social-link {
  ...
  gap: 0.5rem;
  padding: 10px 15px;
  background-color: ${config.theme.style === 'dark' || config.theme.style === 'gaming' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.05)'};
  border-radius: var(--border-radius);
  text-decoration: none;
  color: var(--text-color);
  transition: var(--transition);
...
.video-card {
  background-color: ${config.theme.style === 'dark' || config.theme.style === 'gaming' ? 'rgba(255, 255, 255, 0.05)' : 'white'};
  border-radius: var(--border-radius);
  overflow: hidden;
  box-shadow: var(--box-shadow);
  transition: var(--transition);
...
.twitch-section {
  padding: 5rem 0;
  background-color: ${config.theme.style === 'dark' || config.theme.style === 'gaming' ? 'rgba(255, 255, 255, 0.02)' : 'rgba(0, 0, 0, 0.02)'};
}
...
.live-videos-section {
  padding: 5rem 0;
  background-color: ${config.theme.style === 'dark' || config.theme.style === 'gaming' ? 'rgba(255, 255, 255, 0.01)' : 'rgba(0, 0, 0, 0.01)'};
}
...
const CONFIG = {
  ...
  streaming: {
    twitch: {
      enabled: ${config.streaming?.twitch?.enabled || false},
      username: '${config.streaming?.twitch?.username || ''}',
      showChat: ${config.streaming?.twitch?.showChat || false}
    }
  },
  contactForm: {
