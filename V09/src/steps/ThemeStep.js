...
class ThemeStep extends BaseSetupStep {
  ...
  async setupFonts() {
    ...
    this.uiHandler.displayOptions(fontOptions, 'Choose your font style');
    const fontChoice = await this.uiHandler.selectFromOptions(fontOptions, 'Select font option (1-6): ');
  ...
  async setupTheme() {
    ...
    this.uiHandler.displayOptions(styleOptions, 'Choose your visual theme');
    const styleChoice = await this.uiHandler.selectFromOptions(styleOptions, 'Select theme option (1-7): ');
    ...
    if (styleChoice === 6) { // Custom colors
      ...
      this.uiHandler.displaySuccess(`${styleOptions[styleChoice].split(' ')[0]} theme configured!`);
    }
  }
  ...
  async setupCustomColors() {
    ...
    const primaryColor = await this.uiHandler.getValidHexColor('Primary color (buttons, headings)', '#3498db');
      this.setConfig('theme.colors.primary', primaryColor);

      const secondaryColor = await this.uiHandler.getValidHexColor('Secondary color (hover effects)', '#2ecc71');
      this.setConfig('theme.colors.secondary', secondaryColor);

      const backgroundColor = await this.uiHandler.getValidHexColor('Background color', '#ffffff');
      this.setConfig('theme.colors.background', backgroundColor);

      const textColor = await this.uiHandler.getValidHexColor('Text color', '#333333');
      this.setConfig('theme.colors.text', textColor);

      const accentColor = await this.uiHandler.getValidHexColor('Accent color (highlights)', '#e74c3c');
      this.setConfig('theme.colors.accent', accentColor);

      const customFontColors = (await this.uiHandler.question(colors.cyan + 'Do you want to customize font colors? (y/n): ' + colors.reset)).toLowerCase() === 'y';
      if (customFontColors) {
        const headingColor = await this.uiHandler.getValidHexColor('Heading font color', '#2c3e50');
        this.setConfig('theme.colors.headingText', headingColor);

        const bodyTextColor = await this.uiHandler.getValidHexColor('Body text color', '#333333');
        this.setConfig('theme.colors.bodyText', bodyTextColor);

        const linkColor = await this.uiHandler.getValidHexColor('Link color', '#3498db');
        this.setConfig('theme.colors.linkText', linkColor);
    } else {
    ...
    this.uiHandler.displaySuccess('Custom color scheme configured!');
  }
}
